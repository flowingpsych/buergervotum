 ...
 * - removed unneeded shortcut methods ccm.set and ccm.del
 * - loaded HTML is not automatically converted to JSON for better performance
 * - ccm.load.timeout moved to ccm.timeout
 * - added attribute 'async' for loading of resources via <script>
 * - removed property 'import' for resource objects
 * - renamed value 'data' to 'json' for 'method' property of a resource object
 * - added loading of xml regardless of the file extension
 * - loading of a subset of exported module members
 * - ccm.helper.html accepts functions that return a TemplateResult from lit-html
 * - each instance knows her children instances
  ...
  const ccm = {
      ...
      return new Promise( ( resolve, reject ) => {
        ...
        args.forEach( ( resource, i ) => {
          ...
          function loadModule() {
            ...
            window.ccm.callbacks[ callback ] = result => {
              element.parentNode.removeChild( element );
              delete window.ccm.callbacks[ callback ];
              successData( result );
            };
            let element = ccm.helper.html( { tag: 'script', type: 'module', async: true } );
            if ( resource.attr ) element = Object.assign( element, resource.attr );
            element.onerror = event => { element.parentNode.removeChild( element ); error( element, event ); };
            const [ url, ...keys ] = resource.url.split( '#' );
            const key = keys.join( ',' );
            const query = keys.length === 1 ? `{${key} as result }` : `* as result`;
            const result = keys.length > 1 ? `(({${key}}) => ({${key}}))(result)` : 'result';
            element.text = `import ${query} from '${url}'; window.ccm.callbacks['${callback}'](${result})`;
    ...
    helper: {
      ...
      isSubset: function ( obj, other ) {
        ...
        other = ccm.helper.toDotNotation( other, true );
        for ( const key in obj )
